...
public interface GroupAchieveRepo extends JpaRepository<GroupAchieve, UUID> {
    @Query(value = "SELECT g.id, g.group_achievement_name, g.percentage ,count(a.id) as count, g.enabled " +
            "from group_achievement g join achievement a " +
            "on g.id = a.group_achievement_id AND a.enabled = 1 " +
            "group by g.id, g.group_achievement_name;",
            nativeQuery = true)
